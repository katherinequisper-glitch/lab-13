...
                        @endif
                        ...
                        {{-- ======================================================= --}}
                        {{-- INICIO: CÓDIGO AGREGADO PARA ACTIVIDADES Y ELIMINACIÓN --}}
                        {{-- ======================================================= --}}
                        <hr style="margin-top: 15px;">
                        <h4>Actividades Pendientes:</h4>

                        <ul>
                            @forelse ($nota->actividads as $actividad)
                                <li style="list-style: none;">
                                    
                                    <form method="POST" action="{{ route('actividads.update', ['nota' => $nota, 'actividad' => $actividad]) }}" style="display: inline;">
                                        @csrf
                                        @method('PATCH')
                                        
                                        <button type="submit" style="background:none; border:none; cursor:pointer; color: blue; margin-right: 5px;">
                                            [ {{ $actividad->completada ? '✅' : '⬜' }} ]
                                        </button>
                                    </form>
                                    
                                    <span style="{{ $actividad->completada ? 'text-decoration: line-through; color: gray;' : '' }}">
                                        {{ $actividad->descripcion }}
                                    </span>
                                </li>
                            @empty
                                <p style="font-style: italic;">No hay actividades añadidas para esta nota.</p>
                            @endforelse
                        </ul>

                        <form method="POST" action="{{ route('actividads.store', $nota) }}" style="margin-top: 10px;">
                            @csrf
                            <input type="text" name="descripcion" placeholder="Nueva actividad..." required 
                        ...
                        {{-- 3. BOTÓN DE ELIMINAR NOTA (CON CONFIRMACIÓN Y CASCADA) --}}
                        <form method="POST" action="{{ route('notas.destroy', $nota) }}" 
                            onsubmit="return confirm('¿Estás SEGURO de eliminar la nota {{ $nota->titulo }}? Se eliminarán también recordatorios y actividades.');"
